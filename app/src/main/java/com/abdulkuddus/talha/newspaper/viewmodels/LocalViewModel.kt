...
/**
 * LocalViewModel is designed to store and manage UI-related lottie_onboarding_setup in a lifecycle conscious way. This
 ...
 */
class LocalViewModel(private val repository: NewsRepository, app: Application) : AndroidViewModel(app) {
    ...
    val news: LiveData<NewsWithStatus> = repository.getLiveNews(NewsCategories.LOCAL)
    ...
    fun refreshNews() {
        ...
        viewModelScope.launch {
            repository.refreshNews(getApplication(), NewsCategories.LOCAL)
        }
    }
    ...
    /**
     * Factory for constructing [LocalViewModel] with parameter
     */
    class Factory(val repository: NewsRepository, val app: Application) : ViewModelProvider.Factory {
        override fun <T : ViewModel?> create(modelClass: Class<T>): T {
            if (modelClass.isAssignableFrom(LocalViewModel::class.java)) {
                ...
                return LocalViewModel(repository, app) as T
            }
